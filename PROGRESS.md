...
### Phase 5 — モンスター職業制（最後の大型機能・未着手）
設計書: `documents/2026_05_06/all/monster_moves_with_range.md`

8 職業 × 約20技ずつ × 6 属性の組み合わせで敵の行動パターンを多様化:
- 獣王 / 武道家 / ホラーマン / コウモリ / スケルトン / 蛇族 / ゾンビ / ドラゴン
- 各職業に固有の戦闘スタイル（範囲特性 / レベル帯 / 行動 AI）
- バーコードの 3 桁目から決定論的に職業を決める想定
- 職業 × タイプ（属性）でモンスター名が動的に変わる:
  例) 火属性 + 獣王 = 「フレイムビースト」, 闇属性 + スケルトン = 「シャドウスケルトン」

実装スコープ案:
1. `src/monster-jobs.js` 新規: `MONSTER_JOBS` 定義（職業ごとの統計補正・好む範囲・固有 AI）
2. `generator.js` の `generateMonster` を職業対応に拡張
3. `dungeon.js` の `tickEnemies` で職業別 AI（ドラゴンは FLOOR 攻撃、コウモリは LINE3 等）
4. ボス職業ごとの究極技（80～100Lv の必殺技）
5. UI: モンスター情報モーダルで職業を表示（任意）
